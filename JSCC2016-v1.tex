...
%*************************************************************************************************************
%-------------------------------------------------------------------------
\Abstract{In our previous work, we gave a necessary and sufficient condition for an semi-algebraic set being an invariant of a polynomial autonomous dynamical system, which gave a confirmative answer to the open problem. In addition, based on which a relatively complete algorithm for generating all semi-algebraic invariants of a given polynomial autonomous hybrid system with the given shape was proposed. In this paper, we consider how to extend our previous work to non-autonomous dynamical and hybrid systems. Non-autonomous dynamical and hybrid systems are with inputs, which are very common in practice; in contrast, autonomous ones are without inputs.
...
It's easy to prove that operations on a semi-algebraic set is equivalent to corresponding operations on the formula defining it. More specifically, if we use $S(\psi)$ to represent semi-algebraic set defined by $\psi$, we have the following properties:
\begin{itemize}
...
\label{def:lieDeri}
For a given function $\phi$, a given vector function $\boldsymbol{f}(t, \boldsymbol{x})$, we define Lie derivatives $\mathcal{L}_{t, f}^k$ inductively as:
\begin{align*}
...
\begin{example}
	...
	\begin{align*}
		...
		\mathcal{L}_{t, f}^1 \phi (x, y) &= 2y^2 - x - 2ty + t \\
		\mathcal{L}_{t, f}^2 \phi (x, y) &= 4y^2 + x - 6ty + 2t^2 - t \\
		\dots & \dots
	\end{align*}
\end{example}
...
Polynomial ideal is a powerful tool when dealing with problems related to polynomials. Here we recall some basic contents about it, they are mainly taken from \cite{cox1992ideals}.
...
A well-known result is every polynomial ideal in real field can be generated by a finite basis:

\begin{theorem}[Hibert's Basis Theorem]
Any polynomial ideal $\boldsymbol{I} \in \mathbb{R}[\boldsymbol{x}]$ can be generated by a finite basis. i.e. for any ideal $\boldsymbol{I} \in \mathbb{R}[\boldsymbol{x}]$, there exists $p_1, p_2, \dots, p_k \in \mathbb{R}[\boldsymbol{x}]$ such that $\boldsymbol{I} = \langle p_1, p_2, \dots, p_k \rangle$.
\end{theorem}
...
For the first two situations, we see that the first $\gamma_{f, \phi}(\boldsymbol{x}_0, t_0)$ terms of the right side equation is equal to 0, and the coefficient of the dominant term has the same sign as $\mathcal{L}_{t_0, f}^{\gamma_{f, \phi}(\boldsymbol{x}_0, t_0)} \phi (\boldsymbol{x}_0)$, so we can immediately get the corresponding conclusions.

As for the last situation, $\gamma_{f, \phi}(\boldsymbol{x}, t_0) = \infty$ implies that every term of the right side expansion is equal to 0, which means $\phi (\boldsymbol{x}(t)) =0$ satisfies in the interval where unique solution exists and $\phi$ is analytic.
\rulex \newline

Note the three situations in Theorem~\ref{thm:lieDeri} include all possible values of $\mathcal{L}_{t_0, f}^{k}$, so an inverse version of this theorem can be given, we state it as a corollary:

\begin{corollary}
\label{cor:lieDeri}
For a sequence $\{ t_i \}$  satisfying $t_i > t_0$ and $\lim_{i \to \infty} t_i = t_0$, let $\boldsymbol{x}_0 \doteq \boldsymbol{x}(t_0)$, we have:
\begin{enumerate}
...
\label{def:trans}
	Given a hybrid system $(\boldsymbol{H}, \boldsymbol{I}, \boldsymbol{f})$, we define transverse set of a region $S(p(\boldsymbol{x}) \geq 0)$ as:
	\begin{equation*}
...
\proof
First we consider the necessary part. If the first condition is not satisfied, then we have $(\boldsymbol{I}(\boldsymbol{x}) \geq 0) \nRightarrow (\phi(x) \geq 0)$. That is to say $\boldsymbol{I} \not\subseteq S(\phi(\boldsymbol{x}) \geq 0)$, which contradicts the first condition of Definition~\ref{def:invariant}.

Now if the second condition is not satisfied, then: there exists $\boldsymbol{x}_0$ and $t_0 > 0$ such that $\phi(\boldsymbol{x_0}) = 0$ and $\boldsymbol{x}_0 \in \mathrm{Trans}_{f \uparrow \phi}^{(t_0)} \cap (\mathrm{Trans}_{f \uparrow H}^{(t_0)})^c$. By Definition~\ref{def:trans} and Theorem~\ref{thm:lieDeri} we know:
\begin{equation*}
...
\begin{enumerate}
	\item $\gamma_{f, \boldsymbol{H}}(\boldsymbol{x}_m, t_m) < \infty$ and $\mathcal{L}_{t_m, f}^{\gamma_{f, \boldsymbol{H}}(\boldsymbol{x}_m, t_m)} \boldsymbol{H} (\boldsymbol{x}(t_m)) > 0$, or:
	\item $\gamma_{f, \boldsymbol{H}}(\boldsymbol{x}_m, t_m) = \infty$;
\end{enumerate}
Either way, we have: $\boldsymbol{x}_m \notin \mathrm{Trans}_{f \uparrow H}^{(t_m)}$.

But $\phi(\boldsymbol{x}_m) = 0$, that contradicts the second condition of this theorem. The sufficient part is proved.
\rulex \newline

\subsection{Ideals Generated by Lie Derivatives}
\label{sub:original}
An unsatisfying part Definition~\ref{def:trans} is that it involves infinity. This subsection aims to prove that either the rank of Lie derivative is infinity, or it's less than a computable upper-bound $N$. The main tool used will be ideals generated by Lie derivatives:

\begin{definition}[Ideal Generated by Lie Derivatives]
\label{def:lieIdeal}
Given a polynomial hybrid system $(\boldsymbol{H}, \boldsymbol{I}, \boldsymbol{f})$ and a polynomial $\phi(\boldsymbol{x})$, in $\mathbb{R}[\boldsymbol{x}]$ we call:
		\begin{equation*}
...
So we have: for any $t \in \mathbb{R}$, any $l \geq N$, $\mathcal{L}_{t, f}^l \phi  \in \boldsymbol{I}_N^{(t)}$ holds. That immediately leads us to the final conclusion.
\rulex \newline
...
\begin{displaymath}
	\boldsymbol{I} = \{ \sum_i g_im_i \ |\ g_i \in \mathbb{R}[\boldsymbol{x}], m_i \in M \}
\end{displaymath}
where $M$ is a set of monomials in $\mathbb{R}[\boldsymbol{x}]$.

For simplicity, we use $\langle M \rangle$ to denote the ideal defined above.
\end{definition}
...
Using the notion of \emph{template}, we can derive an algorithm to generate invariants of hybrid systems from the algorithm presented in last subsection. A template is a parametrized polynomial $p(\boldsymbol{u}, \boldsymbol{x}) \in \mathbb{R}[u_1, u_2, \dots, u_m, x_1, x_2, \dots, x_n]$ where $u_1, u_2, \dots, u_m$ are parameters. Our target is to find an invariant $\phi(\boldsymbol{x})$ such that for some $\boldsymbol{u}_0 \in \mathbb{R}^m$, $p(\boldsymbol{u}_0, \boldsymbol{x}) = \phi(\boldsymbol{x})$, or report an error if no such invariant exists.

First we notice that Theorem~\ref{thm:lieIdeal} and related corollaries can be extended to situations where the number of parameters is more than one:
\begin{lemma}
...
\label{sec:general}
In this section, we consider invariant verification and generation for general polynomial hybrid systems. The domain and initial set will be:
\begin{align*}
...
The basic idea is almost the same as that of last section. However, the complex boundaries of invariant, domain and initial set require some extra discussion. Strict proofs are often omitted when they are easy to verify or simply repeats of what we have done in last section.
...
Intuitively, $\boldsymbol{x}_0 \in \mathrm{In}_f^{(t_0)}(A)$ means that any trajectories that pass $\boldsymbol{x}_0$ at $t_0$ must stay in $A$ for a positive time. While $\boldsymbol{x}_0 \in \mathrm{IvIn}_f^{(t_0)}(A)$ means that at least one trajectory that passes $\boldsymbol{x}_0$ at $t_0$ comes from $A$.
...
First let's consider the sufficient part. Recall Definition~\ref{def:invariant}, if ${\it \Psi}$ is not an invariant, then there exists $t_0 \geq 0$, $\boldsymbol{x}_0 \in {\it \Psi}$ and $T_0 \geq 0$ such that:
\begin{equation*}
...
\end{equation*}
Take $t_c \in [t_0, t_0 + T_0]$ such that $\boldsymbol{x}(t_c) \notin {\it \Psi}$, and let
\begin{equation*}
...
%\mathrm{In}_f^{(t_m)}(\boldsymbol{H}) part seems a little strange, only it needs the uniqueness of trajectory.
...
\end{equation*}
Recall Definition~\ref{def:ivin}, that is to say: there exists a trajectory $\boldsymbol{x}$ satisfying $\boldsymbol{x}(t_0) = \boldsymbol{x}_0$ such that: $\exists \epsilon > 0 \forall t \in (t_0 - \epsilon, t_0) : \boldsymbol{x}(t) \in {\it \Psi}$, which also contradicts the second condition of Definition~\ref{def:invariant}.
\rulex \newline
...
\end{enumerate}
\end{theorem}
% ivin parts also need the uniqueness of trajectory
\proof
...
Then, let's consider inward set and inverse inward set of sets defined by a single polynomial. For convenience, we write $\mathrm{In}_f^{(t)}(\phi \rhd 0)$,  $\mathrm{IvIn}_f^{(t)}(\phi \rhd 0)$ to denote $\mathrm{In}_f^{(t)}(S(\phi \rhd 0))$ and $\mathrm{IvIn}_f^{(t)}(S(\phi \rhd 0))$. We have the following theorem: 

\begin{theorem}
\label{thm:polyInIvin}
Given a hybrid system $(\boldsymbol{H}, \boldsymbol{I}, \boldsymbol{f})$ and a polynomial $\phi$, then:
\begin{itemize}
...
\begin{theorem}
\label{thm:semiLie}
Given a hybrid system $(\boldsymbol{H}, \boldsymbol{I},  \boldsymbol{f})$ and a semi-algebraic set $A$:
\begin{equation*}
...
\proof
Use the same technique as Theorem~\ref{thm:translate} to translate the conclusion of Theorem~\ref{thm:polyInIvin} into finite length formulas.
\rulex \newline

Now for general semi-algebraic invariants:
\begin{theorem}
\label{thm:semiCond}
Given a hybrid system $(\boldsymbol{H}, \boldsymbol{I}, \boldsymbol{f})$ and a semi-algebraic set ${\it \Psi}$ where:
\begin{align*}
...
We have: ${\it \Psi}$ is an invariant of the hybrid system if and only if the following formulas:
\begin{align*}
